...
// Função para desenhar o dígito do MNIST em imagem 28x28
void ssd1306_draw_mnist(ssd1306_t *ssd, const uint8_t *img28x28, uint8_t x0, uint8_t y0) {
    for (int y = 0; y < 28; y++) {
        for (int x = 0; x < 28; x++) {
            uint8_t p = img28x28[y * 28 + x];

            // limiar para binarização
            bool on = (p > 80);

            ssd1306_pixel(ssd, x0 + x, y0 + y, on);
        }
    }
}